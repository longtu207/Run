...
var Actor3D = jsb.Sprite3D.extend({
	...
	set2DBody : function(bodyRect){
		this._bodyRect = bodyRect;
	},
	
	
	get2DBodyRect:function(){
		
//		cc.log("x : "+this._bodyRect.x+"x : "+this.getBoundingBox().x);
		
		if(this._bodyRect==null){
			return this.getBoundingBox();
		}
		
		return	cc.rect(this._bodyRect.x+this.x,this._bodyRect.x+this.y,this._bodyRect.width,this._bodyRect.height);
	},
	
	getBodyRect:function(){
		
//		if(!this._bodyCube){
//			this._bodyCube = cc.math.obb(this.getAABB());
//		}
...
//		this._darwNode.drawCircle(cc.p(this.getBodyRect().x, this.getBodyRect().y), 10, 360, 10, true, 5, cc.color(255, 255, 255, 100));
		...
		return cc.math.obbGetCorners(this._bodyCube);
